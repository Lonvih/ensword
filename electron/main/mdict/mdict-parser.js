...
import { lzo1x } from "./minilzo-decompress";
...
//   var p = new Promise(async (_resolve) => {
...
function sliceThen(file, offset, len) {
  var p = new Promise(async (_resolve) => {
    ...
    // };
    // reader.readAsArrayBuffer(file.slice(offset, offset + len));
    const { buffer } = await fileHandle.read(Buffer.alloc(len), 0, len, offset)
    _resolve(buffer.buffer)
  });
...
function harvest(outcomes) {
  return Promise.allSettled(outcomes).then(function (results) {
    if (results.length === 0) {
...
function parse_mdict(file, ext) {
      ...
      readBlock: function (len, expectedBufSize, decryptor) {
        ...
        if (comp_type === 0) {
          ...
          tmp =
            comp_type === 2
              ? pako.inflate(tmp)
              : lzo1x.decompress(tmp, expectedBufSize, 1308672);
          this.forward(len);
  ...
  function matchKeys(phrase, expectedSize, follow) {
    ...
    return startFrom.then(function ([kdx, idx, list]) {
      list = list.slice(idx);
      _trail = {
        phrase: phrase,
        block: kdx.index,
  ...
  const r1 = _slice(pos, 4);
  const r2 = r1.exec(read_file_head);
  return r2
    .then(function (res) {
      const [len] = res;
      // return _slice(pos, 4)
    ...
    .then(function ([header_remain_len, input]) {
    ...
    .then(function ([keyword_summary, keyword_index]) {
